...
<div class="mb-3 col-sm-6 col-md-4 item" :class="{'list-group-item': displayList}">
  <div class="thumbnail card">
    <div class="img-event intrinsic">
      <img :src="backend_url + '/uploads/produit/' + item.id + '/' + item.img" alt="" class="grow thumbnail-image card-img-top intrinsic-item p-3">
    </div>
    <div class="card-body">
      <router-link :to="'/product/' + item.id" tag="h5" class="card-title"><a>{{ item.libelle }}</a></router-link>
      <h6 class="card-subtitle mb-2 remain">{{ item.maxqty}} restants</h6>

      <p class="card-text truncate">{{ item.description | shortDescription}}</p>

      <div class="row">
        <p class="col-6 lead">{{ item.prix }}€</p>
        <p class="col-6">
